...
public class CLI extends AbstractAction {
  ...
  private static String appname = "cliqs"; // name of the properties file to load
	...
	private void populateSymbolTable() {
		...
		if (date != null) {
			cal.setTime(date);
			_symbolTable.put("nowDate", formatDate(date));
			_symbolTable.put("nowTime", formatTime(date));
			_symbolTable.put("nowDateTime", formatDateTime(date));
	...
	private void processArgs(final String[] args) {
		...
		try {
			...
			help.printHelp("Get NOUN [options]" + LF + "Try Get HELP to get more help." + LF + "Here are some command line options:", options);
			System.exit(1);
		}

		if (NOUN == null) {
			exit("Get NOUN [options]" + LF + "Try Get HELP to get more help.", 1);
		}
	...
	public static void main(final String[] args) {
		...
		try {
			...
			debug("Action completed - execution time " + formatElapsed(elapsed));
