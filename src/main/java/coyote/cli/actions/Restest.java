 ...
 * 
 */
public class Restest extends RestAction {
  ...
  public void buildOptions( Options o ) {
    ...
    if ( !o.hasOption( "id" ) ) {
      o.addOption( OptionBuilder.hasArg().isRequired( false ).withArgName( "text" ).withType( String.class ).withDescription( "The identifier to query." ).create( OPT_ID ) );
    }
  }
  ...
  public void validate() throws ActionException {
    ...
    // https://myinstance.service-now.com/api/now/table/{tableName}/{sys_id}

    if ( isDebug() ) {
      debug( "Getting data " + incidentId );
  ...
  public void execute() throws ActionException {
    ...
    // response
    try {
      String result = super.httpGet( scheme, host, port, uri, user, password );
