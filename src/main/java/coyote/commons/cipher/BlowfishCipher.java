...
/**
 * BlowfishCipher is an implementation of the Blowfish 64-bit symmetric block 
...
public class BlowfishCipher extends AbstractCipher implements Cipher {
  ...
  private void blowfishDecrypt( final byte[] in, final int off, final byte[] out, final int outOff ) {
    ...
    L ^= P[BlowfishCipher.ROUNDS + 1];
    for ( int i = BlowfishCipher.ROUNDS; i > 0; ) {
      a = ( L >>> 24 ) & 0xFF;
  ...
  private void blowfishEncrypt( final byte[] in, final int off, final byte[] out, final int outOff ) {
    ...
    L ^= P[0];
    for ( int i = 0; i < BlowfishCipher.ROUNDS; ) {
      a = ( L >>> 24 ) & 0xFF;
      b = 0x100 | ( ( L >>> 16 ) & 0xFF ); // 256 +
      c = 0x200 | ( ( L >>> 8 ) & 0xFF ); // 512 +
      d = 0x300 | ( L & 0xFF ); // 768 +
  ...
  /**
   * @see coyote.commons.cipher.Cipher#decrypt(byte[])
   */
  @Override
  public byte[] decrypt( final byte[] data ) {
    ...
    for ( int i = 0; i < blockCount; i++ ) {
      blowfishDecrypt( data, inOff, retval, i * BlowfishCipher.BLOCK_SIZE );
  ...
  /**
   * @see coyote.commons.cipher.Cipher#encrypt(byte[])
   */
  @Override
  ...
  public int getBlockSize() {
    return BlowfishCipher.BLOCK_SIZE;
  }
  ...
  private synchronized void makeKey( final byte[] key ) throws IllegalArgumentException {
    ...
    if ( len > BlowfishCipher.MAX_USER_KEY_LENGTH ) {
      len = BlowfishCipher.MAX_USER_KEY_LENGTH;
    }
    ...
    System.arraycopy( BlowfishCipher.S0, 0, sKey, 0, 256 );
    System.arraycopy( BlowfishCipher.S1, 0, sKey, 256, 256 );
    System.arraycopy( BlowfishCipher.S2, 0, sKey, 512, 256 );
    System.arraycopy( BlowfishCipher.S3, 0, sKey, 768, 256 );

    int ri;
    for ( int i = 0, j = 0; i < ( BlowfishCipher.ROUNDS + 2 ); i++ ) {
      ri = 0;
      for ( int k = 0; k < 4; k++ ) {
        ri = ( ri << 8 ) | ( kk[j++] & 0xFF );
        j %= len;
      }
      P[i] = BlowfishCipher.Pi[i] ^ ri;
    }

    // use the former to effectively generate the session key
    makeSessionKey( 0, 0, P, 0 );
    for ( int i = 2; i < ( BlowfishCipher.ROUNDS + 2 ); i += 2 ) {
      makeSessionKey( P[i - 2], P[i - 1], P, i );
    }

    // and this session s-boxes
    makeSessionKey( P[BlowfishCipher.ROUNDS], P[BlowfishCipher.ROUNDS + 1], sKey, 0 );
